...
public:
    ...
    {
        if(response->success){
            RCLCPP_INFO_STREAM(_node->get_logger(),  "Trigger request to " << _server_name << " completed\n" << response->message);
            return BT::NodeStatus::SUCCESS;
        } else{
            RCLCPP_WARN_STREAM(_node->get_logger(),  "Trigger request to " << _server_name << " failed\n" + response->message);
